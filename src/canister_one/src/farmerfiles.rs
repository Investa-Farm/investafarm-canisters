...
fn uploaded_farmer_report(
    ...
    match FARM_REPORTS.with(|reports| reports.borrow_mut().insert(farmer_id, report)) {
        Some(_) => Err(Error::Error {
            msg: format!("A report with this farmer id already exists"),
        }),
        None => Ok(Success::ReportUploadedSuccesfully {
            msg: format!("Report uploaded successfully"),
        }),
    }
}

#[update]
pub fn upload_farm_report(farmer_id: u64, report: FarmerReport) -> Result<Success, Error> {
    FARM_REPORTS.with(|reports| {
        let mut reports = reports.borrow_mut();
        reports.insert(farmer_id, report);
